...
b = np.array([[4,7,5]]).T
    \end{lstlisting}\pause
    \item Get the matrix inverse; we use the SciPy package:
    \begin{lstlisting}
...
solution = Ainv.dot(b) 
    \end{lstlisting}\pause
    \item Recommended approach: use the dedicated linear solver:
    \begin{lstlisting}
solution_generic = linalg.solve(A,b)
    \end{lstlisting}
