function [x,y,T,A,b] = solveLaplaceEq(Nx,Ny)
...
tic
% x = A\b;                                    % Solve matrix
[x,it] = solveJacobi(A,b,1e-1);
toc

T = reshape(x,[Nx,Ny]);                     % Reshape x-vec to mat Nx,Ny
[x y] = meshgrid(1:Nx,1:Ny);                % Get position arrays
surf(x,y,T)                                 % Surface plot
