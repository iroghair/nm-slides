      ...
      \begin{center}
        \begin{tikzpicture}
          \begin{axis}[width=1.2\columnwidth,height=4cm,
            ...
            \draw[smooth] (axis cs:2.5,2.3) ellipse [x radius=1.20, y radius=1.50];
          \end{axis}
        \end{tikzpicture}
      \end{center}
    }
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \begin{multline*}
      \frac{dc_1}{dt} = \mathcal{D}\frac{{\color<2>{tuegreen}c_L}-2c_1+c_2}{\Delta x^2},\frac{dc_2}{dt} = \mathcal{D}\frac{c_1-2c_2+c_3}{\Delta x^2},\\ 
      \frac{dc_3}{dt} = \mathcal{D}\frac{c_2-2c_3+c_4}{\Delta x^2},\frac{dc_4}{dt} = \mathcal{D}\frac{c_3-2c_4+{\color<2>{tuegreen}c_R}}{\Delta x^2}
    \end{multline*}
  \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Loop over the internal cells
    \item Set the coefficients in matrix \lstinline|A| (main diagonal + elements left/right to it)
    \item Then set the coefficients for the boundary cells
  \end{itemize}
  \column{0.55\textwidth}
  Set matrix using bands:
  \begin{itemize}
    \item Consider the matrix structure (previous slide) and create vectors containing the values in each band
    \item Recall the \lstinline|sp.sparse.diags| function to set entire bands to a sparse matrix
  \end{itemize}  
  \end{columns}
\end{frame}
