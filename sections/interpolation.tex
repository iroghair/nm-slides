...
\begin{frame}
  ...
  \begin{columns}
	...
	\coordinate (O) at (0,0);
	\node[interp,fill=tuesteel] (x0) at (0,0) {};
	\node[interp,fill=tuesteel] (x1) at (1,0) {};
	\node[interp,fill=tuesteel] (x2) at (1,1) {};
	\node[interp,fill=tuesteel] (x3) at (0,1) {};
	\node<3->[interp,draw=tuegreen,fill=tuegreen] (g1) at (0.75,1) {};
	\node<4->[interp,draw=tuegreen,fill=tuegreen] (g2) at (0.75,0) {};
	
	\node[interp,draw=scharlaken,fill=scharlaken] (p) at (0.75,0.6) {};
...
\begin{frame}
  ...
  \begin{columns}
    \column{0.45\textwidth}
      ...
      \colorize<4>\color<4->{tueorange}  \[ p_2(x) = \frac{7x^2}{6}-7\frac{1}{2}x+13 \]
    ...
    \begin{tikzpicture}[domain=-1:6]
      ...
      \draw<4-> [graph,domain=2:4,draw=tueorange,opacity=0.7] plot (\x, {(7/6)*\x*\x-7.5*\x + 13});
...
\begin{frame}[fragile]
  ...
  \footnotesize\selectfont
     \begin{columns}
    \column{0.45\textwidth}
    Develop the {\color<5->{tuered}$p_2(x)$}, {\color<6->{tuedgreen}$p_3(x)$} and {\color<7>{tueorange}$p_4(x)$} from the following data set (example data \lstinline$x2$ and \lstinline$y2$): 
    \vspace*{-1ex}
    \begin{center}
      \begin{tabular}{c|r}
	$x_k$ & $y_k$ \\ \hline
...
%       \draw (2.5,5) node[above]{$f(x)=\frac{x^3}{2}-\frac{10x^2}{3}+\frac{11x}{2}+1$};
      ...
      % Plots
       ...
       \draw<7> [graph,draw=tueorange,opacity=0.7,domain=-1:1] plot (\x, {32.9234*\x*\x*\x*\x+59.8267*\x*\x*\x+50.8477*\x*\x+43.0766*\x+22.0000});
    \end{tikzpicture}
  \end{columns}
  \pause \vskip1ex
