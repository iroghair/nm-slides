...
\begin{frame}
  ...
  \begin{align*}
    ...
    &= \frac{\mathcal{D}_{i+\frac{1}{2}}c_{i+1} - \left( \mathcal{D}_{i+\frac{1}{2}} + \mathcal{D}_{i-\frac{1}{2}}\right) c_i + \mathcal{D}_{i-\frac{1}{2}} c_{i-1}}{\Delta x}
  \end{align*}
  ...
  Plotting the solution at $t=\left\{12.5,62.5,125,625,5000\right\}$ \si{\second}.
      \begin{tikzpicture}
        \begin{axis}[every axis/.append style={font=\tiny},
          ...
          % Actual equation
          \addplot[graph,draw=tuered] table [y index={1}] {diff_explicit.dat};
          \addplot[graph,draw=tuelblue] table [y index={2}] {diff_explicit.dat};
          \addplot[graph,draw=tuegreen] table [y index={3}] {diff_explicit.dat};
          \addplot[graph,draw=tueorange] table [y index={4}] {diff_explicit.dat};
          \addplot[graph,draw=tuedblue] table [y index={5}] {diff_explicit.dat};
        \end{axis}    
      \end{tikzpicture}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \includegraphics[width=0.8\textwidth]{spy-utc}
  \end{center}
\end{frame}
  ...
  Plotting the solution at $t=\left\{12.5,62.5,125,625,5000\right\}$ \si{\second}.
      \begin{tikzpicture}
        \begin{axis}[every axis/.append style={font=\tiny},
          ...
          % Actual equation
          \addplot[graph,draw=tuered] table [y index={1}] {diff_implicit.dat};
          \addplot[graph,draw=tuelblue] table [y index={2}] {diff_implicit.dat};
          \addplot[graph,draw=tuegreen] table [y index={3}] {diff_implicit.dat};
          \addplot[graph,draw=tueorange] table [y index={4}] {diff_implicit.dat};
          \addplot[graph,draw=tuedblue] table [y index={5}] {diff_implicit.dat};
        \end{axis}    
      \end{tikzpicture}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{multline*}
      ...
      \Rightarrow c_i^{n+1} = \Fo c_{i-1}^{n} + (1-2\Fo)c_i^n+\Fo c_{i+1}^n + R_i^n
    \end{multline*}
    \pause
    ...
    \frac{c_i^{n+1}-c_i^{n}}{\Delta t} = \mathcal{D} \frac{c_{i-1}^{{\color{tuered}n+1}}-2c_i^{{\color{tuered}n+1}}+c_{i+1}^{{\color{tuered}n+1}}}{\Delta x^2} + R(c_i^{{\color{tuered}n+1}}) \\ 
      \Rightarrow -\Fo c_{i-1}^{n+1} + (1+2\Fo - \left. \frac{dR}{dc}\right|_i^n )c_i^{n+1}-\Fo c_{i+1}^{n+1} = c_i^n + R_i^n
      - \left. \frac{dR}{dc}\right|_i^n c_i^n
    \end{multline*}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  Central discretization: $\dfrac{dc}{dx} = \dfrac{c_{i+1}-c_{i-1}}{\Delta x}$ \\
...
\begin{frame}
  ...
  %       \tikzset{at/.style=}
      \begin{axis}[every axis/.append style={font=\tiny},
        ...
        % Actual equation
        \only<3->{\addplot[graph,draw=tuered] table [y index={2}] {conv_cendiff.dat};}
        \only<3->{\addplot[graph,sharp plot,draw=tuered,dashed] table [y index={2}] {conv_ex.dat};}
        \only<4->{\addplot[graph,draw=tueblue] table [y index={4}] {conv_cendiff.dat};}
        \only<4->{\addplot[graph,sharp plot,draw=tueblue,dashed] table [y index={4}] {conv_ex.dat};}
        \only<5->{\addplot[graph,draw=tuegreen] table [y index={6}] {conv_cendiff.dat};}
        \only<5->{\addplot[graph,sharp plot,draw=tuegreen,dashed] table [y index={6}] {conv_ex.dat};}
      \end{axis} 
      \end{tikzpicture}
    }
\end{frame}
...
\begin{frame}
  ...
  %     \draw[help lines] (0,0) grid (6,6);
	...
	\draw[line,-,path fading=custom fade,top color=white!80,bottom color=tuelblue!80,] 
  ...
  \end{array} \right.$
  \pause
  Stable if $\mathit{Co} = \frac{u\Delta t}{\Delta x} < 1$ (with $\mathit{Co}$ the Courant number). However, only $1^\text{st}$ order accurate (large smearing of concentration fronts). Higher order upwind requires TVD schemes (trick of the trade)...
\end{frame}
...
\begin{frame}
  ...
  \begin{array}{l l}
    c_i^{n} -u\dfrac{c_i-c_{i-1}}{\Delta x} & \quad \text{if $u\geq 0$}\\
    \phantom{.}\\
    c_i^{n}-u\dfrac{c_{i+1}-c_i}{\Delta x} & \quad \text{if $u < 0$}\\
  \end{array} \right.
  \end{align*}
\end{frame}
  ...
  \begin{columns}
	...
	\begin{tikzpicture}
	  \begin{axis}[every axis/.append style={font=\tiny},
	    ...
	    \only<2->{\addplot[graph,draw=tuered] table [y index={2}] {conv_upwind1.dat};}
	    \only<2->{\addplot[graph,sharp plot,draw=tuered,dashed] table [y index={2}] {conv_ex.dat};}
	    \only<3->{\addplot[graph,draw=tueblue] table [y index={4}] {conv_upwind1.dat};}
	    \only<3->{\addplot[graph,sharp plot,draw=tueblue,dashed] table [y index={4}] {conv_ex.dat};}
	    \only<4->{\addplot[graph,draw=tuegreen] table [y index={6}] {conv_upwind1.dat};}
	    \only<4->{\addplot[graph,sharp plot,draw=tuegreen,dashed] table [y index={6}] {conv_ex.dat};}
	  \end{axis} 
	\end{tikzpicture}
	\vskip1em
	...
	\begin{tikzpicture}
	  ...
	  \only<5->{\addplot[graph,draw=tuered] table [y index={2}] {conv_upwind.dat};}
	  \only<5->{\addplot[graph,sharp plot,draw=tuered,dashed] table [y index={2}] {conv_ex.dat};}
	  \only<6->{\addplot[graph,draw=tueblue] table [y index={4}] {conv_upwind.dat};}
	  \only<6->{\addplot[graph,sharp plot,draw=tueblue,dashed] table [y index={4}] {conv_ex.dat};}
	  \only<7->{\addplot[graph,draw=tuegreen] table [y index={6}] {conv_upwind.dat};}
	  \only<7->{\addplot[graph,sharp plot,draw=tuegreen,dashed] table [y index={6}] {conv_ex.dat};}
	\end{axis} 
      \end{tikzpicture}
      \vskip1em
