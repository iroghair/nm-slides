\begin{tikzpicture}
    \begin{axis}[
        ...
        \addplot[domain=-4:7, samples=100, color=blue, thick]{(x-5)/2 + sin(deg(x-3)) + 1 + exp(-x-1)*exp(-x-7)};
         %function as per your requirement
        \draw[gray, dashed, thick] (c0) -- (c0x)   node[black, at start, below] {$x_0$};
        ...
        \draw[gray, dashed, thin] (c0) -- (c1x)   node[gray, midway, above, yshift=0.2ex, xshift=0.5ex] {$f^\prime(x_0)$};
