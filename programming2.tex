...
\begin{frame}[fragile]
  ...
  It should work on your neighbors laptop. Well, that is guaranteed anyway. There is a little caveat though, what if your neighbor is stupid?
  \vspace*{2em}
  \begin{lstlisting}
...
\begin{frame}[fragile]
  ...
  Don’t use stupid variable or function names. Rule of thumb: use verbs for functions and nouns for variables.
  \vspace*{2em}
  \begin{lstlisting}
   % stupid names
   x = 5;
...
\begin{frame}[fragile]
  ...
  \end{itemize}
  \tikz{\node[emphblock, text width=\textwidth]{If you have >15 lines of code, you can probably replace it by one or more functions.};}\vspace*{2em}
  
  \begin{tikzpicture}[node distance=2cm]
    % nodes
    \node[emphblock] (A) at (0, 0) {Input};
    \node[emphblock] (B) at (5, 0) {Function};
    \node[emphblock] (C) at (10, 0) {Output};
    % arrows
    % \draw[>=stealth'] (A.east) -- (B.west) (B.west) -- (C).east;
  \end{tikzpicture}

  In principle, I don’t even care what happens inside the function, as long as it gives the expected output.
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Generalization}
    If you code is working for one problem, it should also work for similar problem, in another company, on another planet. Pro-tip: Separate data from algorithms.\\
